...
function GirisContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (isFirebaseConfigured()) {
      ...
      const user = {
        id: registeredUser?.id || (userRole === 'CARRIER' ? `user_carr_${Date.now()}` : `user_cust_${Date.now()}`),
        email: email || '',
        phone: registeredUser?.phone || '',
        role: userRole,
        fullName: registeredUser?.fullName || (userRole === 'CUSTOMER' ? 'Kullanıcı' : undefined),
        companyName: registeredUser?.companyName || undefined,
        carrierProfileId: userRole === 'CARRIER' ? registeredUser?.carrierId : undefined,
