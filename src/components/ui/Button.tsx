...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'navy' | 'outline' | 'outline-white' | 'ghost' | 'danger' | 'gold';
  size?: 'sm' | 'md' | 'lg';
  isLoading?: boolean;
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantStyles = {
    ...
    navy: 'bg-[#111E38] text-white hover:bg-[#1A2E56] focus:ring-[#111E38] shadow-md shadow-slate-900/15',
    outline: 'border-2 border-slate-300 bg-white text-[#111E38] hover:border-[#F95700]/60 hover:bg-orange-50/40 focus:ring-[#F95700]',
    'outline-white': 'border-2 border-white/30 bg-white/10 text-white hover:bg-white/20 hover:border-white/60 focus:ring-white shadow-sm',
    ghost: 'bg-transparent text-slate-600 hover:bg-slate-100 hover:text-[#111E38]',
